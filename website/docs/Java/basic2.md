...
![thisyuan](assets/thisyuan.jpg)

之前学习过this关键字用来区分局部变量和成员变量重名的, 比如没有this的age直接打印是就近原则打印的10, 而有this的age就是用的成员变量的age

那为什么this可以区分, 它的原理是什么, 在内存中如何表示的?
...
2. 存储类型区别
- 数组可以存基本数据类型, 引用数据类型
- 集合可以存引用数据类型, 如果存基本数据类型需要把基本数据类型变成包装类
